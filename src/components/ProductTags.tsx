...
const ProductTags = ({ formik, categories }: Props) => {
  ...
  const handleChecked = (
    ...
    e.target.value;
    if (e.target.checked) {
      formik.values.tags.indexOf(tag) === -1 &&
      ...
      ind !== -1 &&
        formik.setFieldValue(
          'tags',
          formik.values.tags?.filter((item: string) => item !== tag)
        );
    }
  };
        ...
        {categories
          ...
          .map((tag) => (
            <Tag
              key={tag.name}
              tag={tag.name}
              checked={formik.values.tags.indexOf(tag.name) !== -1}
...
const Tag = ({ tag, onChange, checked }: TagProps) => {
  return (
    <div className="bg-primary/80 rounded-md p-2 flex items-center gap-2">
      ...
      />
      <label htmlFor={tag} className="text-xs text-white font-bold">
        {tag}
      </label>
    </div>
  );
