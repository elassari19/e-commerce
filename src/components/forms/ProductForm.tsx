...
const ProductForm = ({ className, categories, formUpdateData }: Props) => {
  ...
  // color properties
  const [properties, setProperties] = useState<Partial<Properties>[]>(
    formUpdateData?.properties || [{ color: '', quantity: '' }]
  );
  // other properties
  const [optionsproperties, setOptionsProperties] = useState<
    Partial<Properties>[]
  >(formUpdateData?.properties || [{ name: '', value: '' }]);
  ...
  const removeImageHandler = (item: any) => {
    const remove = img.filter(
      (pre: any) => pre.secure_url != item.secure_url && item
    );
    setImages(remove);
  };
  ...
  const initialValues = {
    ...
    img,
    categoryId: formUpdateData
      ? toggleIdToSlug(categories, formUpdateData.categoryId)
      : '',
        ...
        <Form className="text-black col-span-12 grid gap-4 p-4 w-full mb-36 md:mb-20 relative">
          {[
            ...
            },
            {
              ...
              accept: 'image/*',
              onChange: (e: any) => {
                uploadImagesHandler(e, setImages),
            ...
            return (
              <div className="grid grid-cols-1 md:grid-cols-12" key={idx}>
                <div className="col-span-full">
                  <label htmlFor={name} className="col-span-full text-md my-4">
                    {lable}
                  </label>
                  {name === 'properties' ? (
                    <>
                      ...
                      <OptionsProperties
                        ...
                        setProperties={setOptionsProperties}
                      />
                    </>
                  ) : name === 'category' ? (
                    <SelectInput
                    ...
                    <ProductTags formik={formik} categories={categories} />
                  ) : (
                    <>
          ...
          <div className="py-8 p-8 fixed bottom-0 right-0 left-0 bg-foreground shadow-md shadow-primary">
            <FormActions
              ...
              update={formUpdateData ? true : false}
            />
          </div>
        </Form>
      )}
