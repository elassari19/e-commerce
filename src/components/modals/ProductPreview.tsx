...
const ProductPreview = ({ product, dialogTrigger, className }: Props) => {
  ...
  return (
    <DialogPopup
      ...
      dialogTrigger={
        dialogTrigger || (
          <div className={cn('cursor-pointer relative', className)}>
            <ShoppingCart
              size={40}
              className="w-11 h-11 text-primary rounded-full p-1 bg-primary/10"
            />
      ...
      dialogContent={
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 h-[30rem] overflow-auto">
          {/* product overview images */}
          <div className="">
            <Tabs defaultValue={'pro2'} className="w-full">
              <div>
                {[
                  ...product?.images,
                  ...product?.properties.filter((p) => p.secure_url),
                ].map((pro, idx) => (
                  <TabsContent
                    key={idx}
                    value={pro + idx.toString()}
          ...
          {/* product details */}
          <div className="flex flex-col gap-4 p-2 md:h-full md:overflow-auto">
            ...
            <div className="flex items-center gap-4 text-sm font-bold">
              ...
              <span>{product.sold || 10} Sold</span>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {/* images tags */}
              <PreviewTabs
                tabList={product.properties
                  .filter((property) => property.color?.length! > 0 && property)
                  .map((property) => (
                    <CartActions
                      key={property.color}
                      product={product}
                ...
                tabContent={product.properties
                  .filter((property) => property.color?.length! > 0 && property)
                  .map((property) => (
                    <p key={property.color} className="text-bold">
                      <strong>Color</strong>: {property.color}
                    </p>
                  ))}
              ...
              <PreviewTabs
                tabList={product.properties
                  .filter((property) => property.name == 'size' && property)
                ...
                tabContent={product.properties
                  ...
                  ?.split(',')
                  .map((size) => (
                    <p key={size} className="text-bold">
          ...
          {/* product order */}
          <div className="border flex flex-col gap-2 p-4 font-bold rounded-lg">
            <h2>Delivery</h2>
            <h3>Free shipping </h3>
            <p>
              <span className="text-primary">8-day delivery</span> by Apr 12
            </p>
            <hr className="border border-primary/70 my-2" />
            <h2>Service</h2>
            ...
            <div className="my-2 w-full">
              {data?.user ? (
                ...
                <DialogPopup
                  dialogTrigger={
                    <Button variant="primary" className="rounded-full w-72">
                      Checkout now
                    </Button>
                  }
            ...
            <div className="flex justify-between items-center gap-4">
              <Button
                ...
                className="rounded-full py-5 w-auto px-4"
              >
